...
$host = 'localhost';
$dbname = 'nom_de_ta_base_de_donnees'; // <--- METS LE NOM DE TA BDD ICI
$user = 'root';
$pass = '';
...
if (isset($bdd)) {
    ...
    $req = $bdd->query("SELECT * FROM objets WHERE statut = 'disponible' AND localisation != ''");
    ...
    $objets_a_afficher = [
        ['id'=>1, 'nom'=>'Chaise Test', 'localisation'=>'Bâtiment E', 'description'=>'Test sans BDD', 'photo'=>'assets/images/logo.svg'],
        ['id'=>2, 'nom'=>'PC Test', 'localisation'=>'IUT', 'description'=>'Test sans BDD', 'photo'=>'assets/images/logo.svg']
    ];
}
?>
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 1. Initialiser la carte (Centrée sur Le Mans)
            var map = L.map('map').setView([48.0163, 0.1665], 15);
            ...
            <?php foreach ($objets_a_afficher as $objet): ?>
                <?php 
                    ...
                    $coords = getCoordonnees($objet['localisation']); 
                ...
                ?>
                    ...
                    var content = `
                        <div class="popup-content">
                            ...
                            <h3><?php echo addslashes($objet['nom']); ?></h3>
                            <p>📍 <?php echo addslashes($objet['localisation']); ?></p>
                            <a href="detaille-objet.php?id=<?php echo $objet['id']; ?>" class="btn-detail">Voir l'objet</a>
                        </div>
                    `;
