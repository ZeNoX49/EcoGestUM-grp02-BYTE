...
AS
BEGIN
    INSERT INTO OBJET (nom_objet, description_objet, date_ajout_objet, 
                       id_utilisateur, id_statut_disponibilite, id_etat, id_categorie)
    VALUES (@p_nom, @p_description, CONVERT(VARCHAR(75), GETDATE(), 120), 
            @p_id_utilisateur, 1, @p_id_etat, @p_id_categorie);
END;
GO
...
BEGIN
    UPDATE OBJET
    SET description_objet = @p_description,
        id_etat = @p_id_etat
...
CREATE OR ALTER PROCEDURE enseignant_creer_evenement
    ...
    @p_type VARCHAR(100),
    @p_description VARCHAR(250),
    @p_date_debut DATETIME,
    @p_date_fin DATETIME,
    @p_id_type_evenement INT,
    @p_id_utilisateur INT
AS
BEGIN
    INSERT INTO EVENEMENT (titre_evenement, type_evenement, description_evenement, 
                          date_debut_evenement, date_fin_evenement, id_type_evenement, id_utilisateur)
    VALUES (@p_titre, @p_type, @p_description, @p_date_debut, @p_date_fin, @p_id_type_evenement, @p_id_utilisateur);
END;
GO